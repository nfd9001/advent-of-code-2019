...
import qualified Data.Matrix as Matrix --matrix on stackage
...
main = do
    ...
    --starting at (1,6) because given the character of the results here, this
    --will keep all indices positive, which Data.Matrix prefers
    map <- robotStep (Map.singleton (1,6) 1) (1,6) DUp m
    let mat = Matrix.matrix 6 43 (\(a,b) -> Map.findWithDefault 0 (b,7-a) map)
    putStrLn $ pp <$> (Matrix.prettyMatrix mat)
